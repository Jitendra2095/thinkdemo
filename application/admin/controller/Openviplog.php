...
class Openviplog extends Admin {

    #|绑定帐号步骤
    public function index(){
        $list = Db::table('user_open_vip_log')
            ->alias('l')
            ->leftJoin('user u','u.id = l.uid')
            ->field('l.*,u.nick_name,u.mobile')
            ->paginate(10);
        $info = $list->items();
    ...
    public function edit($id) {
        $entity = Db::name('user_open_vip_log')->where('id',$id)->find();
        if (!$entity) {
            $this->error('用户对象不存在');
        }
    ...
    public function save(Request $request) {
        ...
        Db::name('user_open_vip_log')->insert(['image'=>$image,'createtime'=>time()]);

        return json(['code' => 0, 'toUrl' => url('/admin/openviplog/index')]);
    }
...
//            return json()->data(['code' => 1, 'message' => "请填写协议内容"]);
        ...
        $data = ['image'=>$image];
        Db::name('user_open_vip_log')->where('id',$id)->update($data);
    ...
    public function delete(Request $request, $id) {
        ...
        $result = Db::name('user_open_vip_log')->where('id',$id)->delete();
        if(!$result){
