...
class Agreement extends Admin {
    ...
    public function index(){
        $list = Db::table('agreement')->select();
    ...
    public function edit($id) {
        $entity = Db::name('agreement')->where('id',$id)->find();
        if (!$entity) {
            $this->error('用户对象不存在');
        }
    ...
    public function save(Request $request) {
        ...
        Db::name('agreement')->insert(['title'=>$title,'content'=>$content,'createtime'=>time()]);

        return json(['code' => 0, 'toUrl' => url('/admin/agreement/index')]);
    }
    ...
    public function update(Request $request, $id) {
        ...
        Db::name('agreement')->where('id',$id)->update($data);
    ...
    public function delete(Request $request, $id) {
        ...
        $result = Db::name('agreement')->where('id',$id)->delete();
        if(!$result){
