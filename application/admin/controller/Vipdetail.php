...
class Vipdetail extends Admin {

    #|绑定帐号步骤
    public function index(){
        $list = Db::table('vipdetail')->select();
    ...
    public function edit($id) {
        $entity = Db::name('vipdetail')->where('id',$id)->find();
        if (!$entity) {
            $this->error('用户对象不存在');
        }
    ...
    public function save(Request $request) {
        ...
        if(empty($image)){
            return json()->data(['code' => 1, 'message' => "请上传图片"]);
        }
        if(empty($content)){
            return json()->data(['code' => 1, 'message' => "请填写协议内容"]);
        }
        Db::name('vipdetail')->insert(['title'=>$title,'image'=>$image,'content'=>$content,'createtime'=>time()]);

        return json(['code' => 0, 'toUrl' => url('/admin/vipdetail/index')]);
    }
    ...
    public function update(Request $request, $id) {
        ...
        $data = ['title'=>$title,'image'=>$image,'content'=>$content];
        Db::name('vipdetail')->where('id',$id)->update($data);
    ...
    public function delete(Request $request, $id) {
        ...
        $result = Db::name('vipdetail')->where('id',$id)->delete();
        if(!$result){
