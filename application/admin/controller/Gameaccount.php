...
class Gameaccount extends Admin {

    #|绑定帐号步骤
    public function index(){
        $list = Db::table('bing_game_account_step')->select();
    ...
    public function edit($id) {
        $entity = Db::name('bing_game_account_step')->where('id',$id)->find();
        if (!$entity) {
            $this->error('用户对象不存在');
        }
    ...
    public function save(Request $request) {
        ...
        if(empty($game_type)){
            return json()->data(['code' => 1, 'message' => "请选择游戏分类"]);
        }
        if(empty($content)){
            return json()->data(['code' => 1, 'message' => "请填写步骤"]);
        }

        Db::name('bing_game_account_step')->insert(['game_type'=>$game_type,'content'=>$content,'createtime'=>time()]);

        return json(['code' => 0, 'toUrl' => url('/admin/Gameaccount/index')]);
    }
    ...
    public function update(Request $request, $id) {
        ...
        $data = ['game_type'=>$game_type,'content'=>$content];
        Db::name('bing_game_account_step')->where('id',$id)->update($data);
